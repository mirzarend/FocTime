...
def open_app():
    ...
    teks_quotes = quotes.random_quotes()
    label_quotes = customtkinter.CTkLabel(halaman_timer, text=teks_quotes, fg_color="transparent", font=("Gill Sans", 20, "bold"), text_color="White", wraplength=500)
...
loading.geometry("350x250")
loading.after(0, lambda: loading.state("zoomed"))

# loading.overrideredirect(True)

progress = customtkinter.CTkProgressBar(master=loading,width=300)
progress.pack(side="bottom", pady= 150)
progress.set(0)
...
def loading_start():
    ...
    progress.set(nilai + (hasil * 0.05))

    if hasil > 0.1:
        loading.after(50, loading_start)
    else:
        loading.withdraw()
        open_app()
        loading.destroy()
